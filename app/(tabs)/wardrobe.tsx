...
// FIX: Imported WardrobeItem interface
import { useWardrobeStore, GARMENT_TEMPLATES, GarmentType, WardrobeItem } from '../../store/wardrobeStore';
...
export default function WardrobeScreen() {
    ...
    const { items, deleteEntireItem, removeColorFromItem, updateColorOfItem } = useWardrobeStore();

    const [selectedCategory, setSelectedCategory] = useState('All');

    const [modalVisible, setModalVisible] = useState(false);
    const [editingItem, setEditingItem] = useState<{ id: GarmentType, colorIndex: number, color: string } | null>(null);
    ...
    const openEditModal = (id: GarmentType, colorIndex: number, color: string) => {
    ...
    // FIX: Added Type 'WardrobeItem' to item prop
    const renderItem = ({ item }: { item: WardrobeItem }) => {
        ...
        return (
            ...
            >
                ...
                <Text style={{ color: colors.inactive, fontSize: 10, textTransform: 'uppercase' }}>
                ...
                <View style={styles.colorRow}>
                    ...
                    {item.colors.map((c, idx) => (
                        <TouchableOpacity
                            ...
                            style={[styles.colorDot, { backgroundColor: c, borderColor: isDark ? '#333' : '#fff' }]}
                        />
                    ))}
                    <TouchableOpacity
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.header}>
                <Text style={[styles.headerTitle, { color: colors.text }]}>Virtual Cupboard</Text>
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyState}>
                        <Text style={{ color: colors.inactive }}>Your cupboard is empty.</Text>
                        <TouchableOpacity onPress={() => router.push('/home' as any)}>
                            <Text style={{ color: colors.primary, marginTop: 10 }}>Scan Items +</Text>
                        </TouchableOpacity>
                    </View>
                }
            />
...
const styles = StyleSheet.create({
    ...
    headerBtn: { padding: 8, backgroundColor: 'rgba(128,128,128,0.1)', borderRadius: 20 },
    ...
    // Card Styles
    card: {
        width: CARD_WIDTH, marginBottom: 16, borderRadius: 16, padding: 12,
        shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3
    },
    svgContainer: { height: 110, alignItems: 'center', justifyContent: 'center', marginBottom: 10 },
    ...
    addDot: { width: 18, height: 18, borderRadius: 9, borderWidth: 1, borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center' },
    ...
    pickerPreview: { height: 40, borderRadius: 8, borderWidth: 1, marginBottom: 10 },

    modalActions: { flexDirection: 'row', gap: 12, marginTop: 24 },
    actionBtn: { flexDirection: 'row', height: 48, borderRadius: 24, alignItems: 'center', justifyContent: 'center', gap: 8, paddingHorizontal: 20 },
