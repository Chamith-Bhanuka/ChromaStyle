...
export interface Outfit {
  ...
  imageUri?: string; // <--- NEW: Stores the selfie photo
...
interface WardrobeState {
  ...
  setOutfitImage: (date: string, uri: string) => void; // <--- NEW ACTION
...
export const useWardrobeStore = create<WardrobeState>()(
  persist(
    (set, get) => ({
      ...
      updateColorOfItem: (id, index, newColor) =>
        set((state) => ({
          items: state.items.map((i) => {
            if (i.id === id) {
              const newColors = [...i.colors];
      ...
      // NEW: Implementation to save selfie image URI
      ...
      autoGenerateWeek: (startDate, useAI) => {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          // Basic Random Logic
          // We check if an outfit already exists to preserve images/custom choices
          if (!newOutfits[dateKey]) {
            ...
            // If it exists, just fill in gaps, don't overwrite existing
            const existing = newOutfits[dateKey];
            newOutfits[dateKey] = {
              ...existing,
              top: existing.top || pickRandom('Tops'),
              bottom: existing.bottom || pickRandom('Bottoms'),
              footwear: existing.footwear || pickRandom('Footwear'),
            };
          }
        }
